-- Visit risk score hooks: call record_risk_signals after CPE debit / unlock.
-- Does not change verification_status or money rules.
-- Reinstall geofence-hardened check-in: debit first, then metadata-only risk.
...
begin
  ...
  if v_event.verification_status = 'verified' then
    ...
    begin
      perform public.record_risk_signals(p_event_id);
    exception when others then
